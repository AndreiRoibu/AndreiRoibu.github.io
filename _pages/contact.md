---
title: "Let's Connect"
permalink: /contact/
layout: single
author_profile: false
---

<p class="lead" style="text-align:left">
I love exchanging ideas at the intersection of AI and business strategy, and across industries ranging from embedded systems to healthcare and aerospace.
If you want to <strong>teach me something new, explore collaboration, or ask for early-career advice</strong>,
let’s talk. The best way to reach me is via email (fastest) or by connecting on LinkedIn.
</p>

<ul style="margin:1rem 0 2.5rem; list-style:none; padding:0; font-size:1.05em">
  <li>🚀 Deep-tech or business collaboration</li>
  <li>🧭 Mentoring / career guidance</li>
  <li>💡 Feedback on my work or site</li>
</ul>

<div style="text-align:center; margin:2em 0 3em">
  <a href="mailto:contact@andreiroibu.com"
     class="btn btn--success btn--large"
  ><i class="fas fa-fw fa-envelope"></i> Email&nbsp;me</a>

  <a href="https://www.linkedin.com/in/andreiroibu/"
     class="btn btn--linkedin btn--large"
     style="margin-left:0.6rem"
  ><i class="fab fa-fw fa-linkedin"></i> LinkedIn</a>
</div>

<p style="text-align:center; font-size:1.1em; border-top:1px solid #444;
          padding-top:2em">
  You can also find my work here:
</p>

<div style="text-align:center; margin:2rem 0 3rem; display:flex;
            flex-wrap:wrap; gap:.75rem; justify-content:center">

  <a href="https://github.com/AndreiRoibu"
     class="btn btn--primary"
     style="min-width:140px">
    <i class="fab fa-fw fa-github"></i> GitHub&nbsp;<small>(Personal)</small>
  </a>

  <a href="https://github.com/AndreiRoibu-synthara"
     class="btn btn--primary"
     style="min-width:140px">
    <i class="fab fa-fw fa-github"></i> GitHub&nbsp;<small>(Work)</small>
  </a>

  <a href="https://git.fmrib.ox.ac.uk/aroibu"
     class="btn btn--primary"
     style="min-width:140px">
    <i class="fab fa-fw fa-gitlab"></i> GitLab&nbsp;<small>(DPhil)</small>
  </a>

</div>

<div style="text-align:center; margin-bottom:3em">
  <img src="/assets/images/contact-photo.jpeg"
       alt="Andrei Roibu"
       style="border-radius:50%; width:240px; height:240px; object-fit:cover">
</div>

<!-- <div style="text-align:center; margin-top:3em">
  <a class="btn btn--info"
     href="https://calendly.com/andreiroibu/intro"
  ><i class="far fa-calendar-alt"></i> Book a 30-min chat</a>
</div> -->
